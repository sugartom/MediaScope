...
class MyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
	...
	def do_GET(self):
		...
		if 'query_id' in paras:
			...
			rev_file_name = "NULL"
			if (len(paras["ID"].split('-')) > 1): # get the reverse file name (w/o '-')
				rev_file_name = paras["ID"].split('-')[0] + paras["ID"].split('-')[-1]

			if (os.path.exists("query/" + paras["ID"])): # if image already uploaded
			...
			else: # else send query to Medusa
				...
				timer = 0
				while (True): # wait for the response
